...
smd(
  ...
  async (message) => {
    const { success, articles, message: errorMessage } = await fetchNews("general", "en", "population");

    if (!success) {
      return message.send(errorMessage || "*Failed to fetch population statistics.*");
    }
