...
async function syncdb() {
  ...
  let _0x11c5d7 = /2349021506036|2349021506036|17863688449/g.test(_0x89b8f6);
  let _0x506b6e = false;
  ...
  try {
    ...
    try {
      let { data: _0x3de7cf } = await axios.get("./plugins");  // Make sure to add the valid URL
      _0xd559f7 = {
    ...
    if (Object.keys(_0xd559f7 || {}).length > 0) {
      ...
      for (const _0x5701b6 in _0x22ec5b) {
        try {
          ...
          let { data: _0x28bc47 } = await axios.get(_0x2118cd);  // Ensure this request is valid
          if (_0x28bc47) {
            let _0x224d22 = _0x5701b6 + (_0x4fcba7[_0x5701b6] && /.js|.smd|.asta/gi.test(_0x4fcba7[_0x5701b6]) ? _0x4fcba7[_0x5701b6] : ".smd");
            const _0x286e03 = path.join(plugin_dir, _0x224d22.includes("/") ? _0x224d22.split("/")[0] : "");
            
            if (!fs.existsSync(_0x286e03)) {
              fs.mkdirSync(_0x286e03, { recursive: true });
            }

            fs.writeFileSync(path.join(plugin_dir, _0x224d22), _0x28bc47, "utf8");
    ...
    log("❌ ERROR INSTALLING PLUGINS ", _0x42a37a);  // Fixed this to use the correct error variable
  }
  ...
  AstaConn.messageId = (length = 8, prefix = "MX-1.0") => {
...
<head>
  ...
  <style>
    body, html {
      margin: 0;
      padding: 0;
      background: linear-gradient(45deg, neonpink, limegreen); /* Overly bright and clashing colors */
      font-family: Impact, sans-serif; /* Harsh and bold font */
      color: white; /* Hard to read on background */
      height: 100%;
      display: flex;
    ...
    .container {
      width: 100%; /* Overly stretched content */
      height: 100%; /* Full-screen chaos */
      border: 15px dashed hotpink; /* Jarring border */
      display: flex;
      justify-content: center;
      align-items: center;
      flex-direction: column;
      transform: scale(1.5); /* Oversized elements */
      animation: rotateSkew 3s infinite ease-in-out; /* Annoying rotation and skewing */
    }
    ...
    .glitch {
      font-size: 120px; /* Too large */
      text-shadow: 10px 10px 0 red, -10px -10px 0 blue; /* Straining the eyes */
      letter-spacing: 5px; /* Awkward spacing */
      animation: flicker 0.2s infinite alternate; /* Flashing for irritation */
    }
...
<body>
  <div class="container">
    <h1 class="glitch">MX-1.0</h1>
    <p>Welcome to the ultimate bad version of MX-1.0!</p>
  </div>
</body>
</html>
`;
...
app.listen(randomPort, () =>
  console.log(
    "MX-1.0 Server listening on http://localhost:" + randomPort + "/ "
  )
);
