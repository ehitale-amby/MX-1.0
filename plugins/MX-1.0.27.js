...
const gfxFunc = async (cld, { Void, text, smd: cmdName }, sendError = true) => {
 try {
   text += ": ser";
   let exampleText = `Example  : *${prefix + cmdName}* mx`;
   let infoText = `*Separate the text with ':' sign*\n*Example : ${prefix + cmdName} mx : Mx*`;
...
smd(
 ...
 async (cld, text, { smd: cmdName, Void }) => {
   try {
     let exampleText = `*Separate the text with _:_ sign!*\n*Example : ${prefix + cmdName} MX-1.0 _:_ Bot*`;
     if (!text) {
       let menu = `┌───〈 *ɢꜰx ᴍᴇɴᴜ*  〉───◆\n│╭─────────────···▸\n┴│▸\n⬡│▸ ${gfxx.join(" \n⬡│▸ ")} \n┬│▸\n│╰────────────···▸▸\n└───────────────···▸\n\n\t *USE: ${prefix + cmdName} MX-1.0*\n_To get All Results with single Cmd!_\n`;
       return await cld.sendUi(cld.chat, { caption: mx });
     }
