...
async (message, match) => {
  try {
    ...
    if (!match) {
      return message.reply(`Please provide a GitHub username, e.g., ${prefix}github themxgamecoder`);
    }

    // Fetch data from the GitHub API
    const { data } = await axios.get(`https://api.github.com/users/${match}`);
    if (!data) {
      return await message.send(`*_No results found. Please provide a valid GitHub username!_*`);
    }
...
🧩 *Created At*: ${new Date(created_at).toLocaleString() || "N/A"}
...
`.trim();
    ...
    return await message.error(
      `${error.message}\n\nCommand: github`,
      error,
      `*_An error occurred. Unable to fetch data for the given username!_*`
    );
  }
});
...
async(message, match) => {
 try{
   ...
   message.react("🔍")
         if (!match) return message.reply(`Give me a user name like ${prefix}github themxgamecoder`)
